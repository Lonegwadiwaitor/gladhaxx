...
const LuaNode luaH_dummynode = *reinterpret_cast<LuaNode*>(dummynode_address);

#define dummynode (reinterpret_cast<LuaNode*>(dummynode_address))
...
static FORCEINLINE LuaNode* hashpointer(const Table* t, const void* p)
{
...
static FORCEINLINE LuaNode* hashnum(const Table* t, double n)
{
...
static FORCEINLINE LuaNode* hashvec(const Table* t, const float* v)
{
...
** of its hash value)
*/
static FORCEINLINE LuaNode* mainposition(const Table* t, const TValue* key)
{
    switch (ttype(key))
    {
    case LUA_TNUMBER:
...
*/
static FORCEINLINE int arrayindex(double key)
{
    int i;
    luai_num2int(i, key);
...
*/
static FORCEINLINE int findindex(lua_State* L, Table* t, StkId key)
{
    int i;
    if (ttisnil(key))
...
static FORCEINLINE int computesizes(int nums[], int* narray)
{
    int i;
    int twotoi; // 2^i
...
static FORCEINLINE int countint(double key, int* nums)
{
...
static FORCEINLINE int numusearray(const Table* t, int* nums)
{
    int lg;
    int ttlg;     // 2^lg
...
static FORCEINLINE int numusehash(const Table* t, int* nums, int* pnasize)
{
...
static FORCEINLINE void setarrayvector(lua_State* L, Table* t, int size)
{
    if (size > MAXSIZE)
...
static FORCEINLINE void setnodevector(lua_State* L, Table* t, int size)
{
    int lsize;
    if (size == 0)
...
static FORCEINLINE TValue* newkey(lua_State* L, Table* t, const TValue* key);

static FORCEINLINE TValue* arrayornewkey(lua_State* L, Table* t, const TValue* key)
{
    if (ttisnumber(key))
    {
        int k;
...
static FORCEINLINE void resize(lua_State* L, Table* t, int nasize, int nhsize)
{
...
static FORCEINLINE int adjustasize(Table* t, int size, const TValue* ek)
{
...
static FORCEINLINE void rehash(lua_State* L, Table* t, const TValue* ek)
{
...
static FORCEINLINE LuaNode* getfreepos(Table* t)
{
    while (t->lastfree > 0)
    {
        t->lastfree--;
...
static FORCEINLINE int updateaboundary(Table* t, int boundary)
{
