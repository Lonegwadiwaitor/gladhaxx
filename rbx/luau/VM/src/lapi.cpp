...
static FORCEINLINE TValue* pseudo2addr(lua_State* L, int idx)
{
...
void lua_clonecfunction(lua_State* L, int idx)
{
    ...
    newcl->c.f = (lua_CFunction)cl->c.f; /* TODO: this would only work on ROBLOX's C closures, add support for ours */
...
std::unordered_map<Closure*, lua_CFunction> temporary_callcheck; // ptsd

int wtf(lua_State* R) {
    auto func = clvalue(R->ci->func);

    if (func->c.debugname != nullptr) {
	    if (static_cast<std::string_view>(func->c.debugname) != "") {

            std::printf("calling %s\n", static_cast<const char*>(func->c.debugname));

            int status = temporary_callcheck[func](R);
...
void __fastcall lua_pushinstance(lua_State* R, Instance* instance) {
    /*
     * .text:100AA6FD                 lea     edx, [ebp+instance] ; 12
     * .text:100AA700                 push    edx
     * .text:100AA701                 mov     eax, [ebp+R] ; 8
     * .text:100AA704                 push    eax
     * .text:100AA705                 call    [ebp+var_10] ; push_instance
     * .text:100AA708                 add     esp, 8
     */
...
int lua_getidentity(lua_State* R) {
    return R->extraspace->identity;
}

void lua_setidentity(lua_State* R, int new_identity) {
    std::uintptr_t teb = (std::uintptr_t)NtCurrentTeb();

    int ThreadLocalStoragePointer = *(int*)*reinterpret_cast<DWORD*>(teb + 0x2C);

    int& v8 = *(int*)(ThreadLocalStoragePointer + 0xD8);

    v8 = new_identity;
    R->extraspace->identity = new_identity;
}
...
void lua_setthreaddata(lua_State* L, ExtraSpace* data)
{
    L->extraspace = data;
}
