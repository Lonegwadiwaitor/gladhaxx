...
int get_top_pos(lua_State* L, TValue* val)
{
    return (L->top - val) - 1;
}
...
//{
//    auto nargs = get_top_pos(L, func);
//
//    auto oldtop = lua_gettop(L) - nargs - 1;
//
//    // do stuff
//
...
//    auto f_call = reinterpret_cast<f_call_t>(mem::rebase(0x197beb0, 0x400000));
//
//    status = reinterpret_cast<int(__fastcall*)(lua_State*, f_call_t, void*)>(mem::rebase(0x196ce90, 0x400000))(L, f_call, c.func);
//
//    //reinterpret_cast<void(__cdecl*)(lua_State*, void*)>(mem::rebase(0x197beb0, 0x400000))(L, func);
//
//    __asm {
//        mov ebp, oebp
//        mov esp, oesp
...
//        return;
//
//    if ((lua_gettop(L) - oldtop) != nresults) // pcallrun hack
//        lua_pop(L, lua_gettop(L) - oldtop);
//
//    std::printf("%i", nresults);
//}
