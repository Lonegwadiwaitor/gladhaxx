...
std::optional<std::string> luaV_getimportname(lua_State* L, Table* env, TValue* k, StkId res, uint32_t id, bool propagatenil)
{
    int count = id >> 30;
    LUAU_ASSERT(count > 0);
    ...
    if (!ttisnil(res)) {
        return std::string(tsvalue(&k[id0])->data);
    }
    ...
    if (!ttisnil(res)) {
        return std::string(tsvalue(&k[id1])->data);
    }
    ...
    if (!ttisnil(res)) {
        return std::string(tsvalue(&k[id2])->data);
    }
...
void resolveImportSafe(lua_State* L, Table* env, TValue* k, uint32_t id)
{
