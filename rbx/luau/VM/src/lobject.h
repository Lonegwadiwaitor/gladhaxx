...
typedef struct Proto
{
    ...
    VM_SHUFFLE4(; ,
        PROTO_CONSTANT_OBF k, // constants used by the function
        PROTO_CODE_OBF code,      // function bytecode
        PROTO_PROTOS_OBF p,       // functions defined inside the function
    ...
    VM_SHUFFLE5(; ,
        PROTO_LINEINFO_OBF lineinfo,      // for each instruction, line number as a delta from baseline
        PROTO_ABSLINEINFO_OBF abslineinfo,       // baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo
        PROTO_LOCVARS_OBF locvars, // information about local variables
        PROTO_UPVALUES_OBF upvalues,     // upvalue names
        PROTO_SOURCE_OBF source);

    PROTO_DEBUGNAME_OBF debugname;
...
typedef struct Closure
{
    ...
    union
    {
        ...
        {
            CLOSURE_FUNC_OBF f;
            CLOSURE_CONT_OBF cont;
            CLOSURE_DEBUGNAME_OBF debugname;
            TValue upvals[1];
        } c;
...
typedef struct Table
{
    ...
    VM_SHUFFLE4(;,
        TABLE_METATABLE_OBF metatable,
        TABLE_ARRAY_OBF array,  // array part
        TABLE_NODE_OBF node,
        GCObject* gclist);
} Table;
// clang-format on
