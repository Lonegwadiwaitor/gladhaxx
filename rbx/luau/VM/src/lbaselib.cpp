...
static void getfunc_internal(lua_State* L, int level = 1)
{
    if (lua_isfunction(L, 1))
        lua_pushvalue(L, 1);
    else
...
int luaB_getfenv(lua_State* L)
{
	...
	if (auto obj = index2addr(L, 1); obj && obj->tt == LUA_TNUMBER)
    {
	    // there's a level.

        auto level = obj->value.n;

        getfunc_internal(L, level);
        auto cl = clvalue(index2addr(L, -1));
        lua_pop(L, 1);

        if (cl->l.p->linegaplog2 == 1337)
        {
	        /* it's ours */
            // fuck.
            // ok, pretend it's not there.

            obj->value.n++; /* up level */
