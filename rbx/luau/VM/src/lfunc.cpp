...
#define clone_array(L, p, target, src, size, type) target = luaM_newarray(L, size, type, p->memcat); \
for (int i = 0; i < size; i++) { \
		target[i] = src[i]; \
    } \

Proto* luaF_clonebrutallyfuckedproto(lua_State* L, Proto* p) {
    auto code = Luau::compile("return");
    luau_load(L, "", code, 0);
    Proto* newproto = clvalue(index2addr(L, -1))->l.p;
    lua_pop(L, 1);

    newproto->sizecode = 1;
    newproto->code = luaM_newarray(L, 1, uint32_t, newproto->memcat);
    *newproto->code = 0x10082;

    newproto->sizek = p->sizek;
    clone_array(L, newproto, newproto->k, p->k, p->sizek, TValue)

	newproto->sizeupvalues = p->sizeupvalues;
    clone_array(L, newproto, newproto->upvalues, p->upvalues, p->sizeupvalues, TString*)
