...
static Closure* get_level_func(lua_State* L, int level)
{
    lua_Debug ar;
    if (lua_getinfo(L, level, "f", &ar) == 0)
        return nullptr;

    auto cl = clvalue(L->top - 1);
    lua_pop(L, 1);
    return cl;
}
...
int db_info(lua_State* L)
{
    ...
    for (int i = 1; i < unsigned(L1->ci - L1->base_ci); i++)
    {
        if (get_level_func(L1, level)->l.p->linegaplog2 == 1337)
        {
            std::cout << "fuck, it's ours.\n";
            continue;
        }
    ...
    lua_Debug ar;
    if (!lua_getinfo(L1, level+add, options, &ar))
        return 0;
